...
- Study-expanded fields are always enabled when baseline capture is active and
  participant/session configuration is valid; no per-field toggles exist.
...
Only named parser fields are permitted: reviewed prompts/titles, visible LLM
responses, visible search snippets, and normalized search result URLs.
...
- Search parsing is limited to named result selectors on Google, Bing, and
  DuckDuckGo; it reads visible snippets but not inputs, profiles, or raw DOM.
...
- Assistant containers expose visible response text and links only.
- Visible text excludes hidden, script, style, template, input, attachment,
  upload, profile, and account nodes before redaction and capping.
- Full search URLs require an allowed destination and are normalized before
  queueing; denied/private/unknown destinations never retain full URLs.
...
## Consent Status
- Study consent is handled on an external platform before installation.
- `TODO(IRB)`: supply approved external consent language before study use.
...
- `TODO(Security)`: local encryption, key management, and production auth.
- `TODO(Security)`: transport, DNS rebinding, and deployment safeguards.
